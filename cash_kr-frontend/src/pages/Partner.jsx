...
import { 
  ...
  Store, Laptop, Wrench, Trash2, Network, AppWindow
} from "lucide-react";
...
const VALUE_PROPS = [
  {
    ...
    desc: "With Cashkr's massive active user base and established online presence, you will receive a consistent structural flow of local device sellers.",
    bullets: ["Pre-qualified leads sent directly to you", "Customers already agree to online quoted price"]
  },
  {
    ...
    bullets: ["All legal paperwork handled by our system", "Real-time automated alerts on Partner App"]
  }
];

const WORKFLOW_STEPS = [
  { num: "1", title: "Download Partner App", desc: "Get the secure Cashkr Partner application build straight from the Google Play Store." },
  { num: "2", title: "Submit Application", desc: "Tap 'Become Partner' inside the dashboard UI and cleanly fill in basic business details." },
  { num: "3", title: "Start Earning Daily", desc: "Enjoy our accelerated KYC validation loop. Gain immediate backend access to nearby marketplace sellers." }
];

const COHORTS = [
  { icon: <Store size={22} className="text-blue-600" />, label: "Mobile Repair Shops", bg: "bg-blue-50" },
  { icon: <Laptop size={22} className="text-purple-600" />, label: "Laptop Retailers", bg: "bg-purple-50" },
  { icon: <Network size={22} className="text-emerald-600" />, label: "Refurbishing Units", bg: "bg-emerald-50" },
  { icon: <Wrench size={22} className="text-amber-600" />, label: "Freelance Technicians", bg: "bg-amber-50" },
  { icon: <Trash2 size={22} className="text-rose-600" />, label: "E-waste Collectors", bg: "bg-rose-50" },
  { icon: <AppWindow size={22} className="text-indigo-600" />, label: "Service Aggregators", bg: "bg-indigo-50" }
];
    ...
    <div className="w-full bg-white antialiased">
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:sticky lg:top-8 mt-4">
            <div className="inline-flex items-center gap-2 bg-white border border-[#0565E6]/20 rounded-full pl-2 pr-4 py-1.5 text-xs font-bold text-[#0565E6] mb-6 shadow-sm">
              ...
              Partner With Cashkr - Device Buyback Solutions
            </div>
            ...
            <p className="text-base text-gray-500 leading-relaxed mb-8 max-w-[540px]">
              Our established infrastructure and highly trusted consumer footprint mean more customers, consistent transactions, and premium revenue pipelines for your local workshop. **Earn up to ₹3,00,000 monthly.**
            </p>
          ...
          <div className="bg-white border border-gray-100 shadow-2xl rounded-[28px] p-6 sm:p-8 relative z-10">
            <div className="mb-6">
              <h3 className="text-lg font-black text-gray-900 mb-1">Become Device Buyback Partner</h3>
              <p className="text-xs text-gray-400">Lock in regional processing priority. Submit your storefront data below.</p>
            </div>
      ...
      <section className="bg-gray-50 border-t border-b border-gray-100 py-10 px-4">
        <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="text-center md:text-left">
            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Global Ecosystem Reach</h4>
            <p className="text-sm font-bold text-gray-600 mt-1">Backed by high conversion ratios and systematic pan-India growth structures.</p>
          </div>
          <div className="grid grid-cols-4 gap-6 sm:gap-12">
            {METRICS.map((m, idx) => (
              <div key={idx} className="text-center">
                <div className="text-xl sm:text-2xl font-black text-[#0565E6]">{m.value}</div>
        ...
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-12 items-center">
          ...
          <div className="bg-gray-900 rounded-[2rem] p-6 sm:p-8 text-white relative shadow-xl overflow-hidden">
            ...
            <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-6">
              <div>
                <span className="text-[10px] bg-red-500 text-white font-black px-2 py-0.5 rounded-md uppercase tracking-wider animate-pulse">Live</span>
                <h4 className="text-base font-black mt-1">Partner App Leads Feed</h4>
              </div>
              <div className="text-right">
                <div className="text-xs text-gray-400">Avg Monthly Leads</div>
                <div className="text-sm font-black text-[#0565E6]">50+ Per Node</div>
              </div>
            </div>
            ...
            <div className="space-y-3.5">
              {LIVE_LEADS.map((lead, idx) => (
                <div key={idx} className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-center justify-between hover:border-white/20 transition">
                  <div>
                    <div className="text-xs font-bold text-white">{lead.title}</div>
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-[9px] text-gray-500 uppercase tracking-wider">Pre-Quoted</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-white/10 mt-6 pt-6 text-center">
              <div>
                ...
                <div className="text-[10px] text-gray-400 uppercase tracking-wide">Consumer App Base</div>
              </div>
              <div>
                <div className="text-lg font-black text-[#0565E6]">50K+</div>
                <div className="text-[10px] text-gray-400 uppercase tracking-wide">Monthly Success Drops</div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-8 mt-6">
              {VALUE_PROPS.map((prop, idx) => (
                <div key={idx} className="flex gap-4">
                  ...
                  <div>
                    <h4 className="text-base font-bold text-gray-900 mb-1.5">{prop.title}</h4>
                    <p className="text-sm text-gray-500 leading-relaxed mb-3">{prop.desc}</p>
                    <div className="flex flex-wrap gap-x-4 gap-y-1.5">
                      {prop.bullets.map((b, bIdx) => (
                        <span key={bIdx} className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-700 bg-gray-50 px-2.5 py-1 rounded-md border border-gray-100">
        ...
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            {WORKFLOW_STEPS.map((step, idx) => (
              <div key={idx} className="bg-white border border-gray-200/60 rounded-2xl p-6 relative flex flex-col justify-between">
                <div>
      ...
      <section className="py-16 sm:py-24 px-4 bg-white">
        <div className="max-w-[1200px] mx-auto">
          <SectionHeading 
            ...
            subtitle="Whether you maintain high-capacity commercial spaces or act as a specialized local technician, Cashkr adapts natively to fuel your business."
          />

          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mt-8">
            {COHORTS.map((c, idx) => (
              <div key={idx} className="border border-gray-100 rounded-xl p-5 text-center bg-white shadow-sm hover:shadow-md transition hover:border-[#0565E6]/30">
                <div className={`w-11 h-11 ${c.bg} rounded-xl flex items-center justify-center mx-auto mb-3.5 shadow-sm`}>
                  {c.icon}
                </div>
                <div className="text-xs font-bold text-gray-800 leading-snug">{c.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-14 px-4 bg-gray-900 text-white rounded-[2rem] max-w-[1200px] mx-auto mb-16 relative overflow-hidden text-center">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4">
          <span className="text-[#0565E6] text-xs font-bold uppercase tracking-widest bg-white/5 px-4 py-1.5 rounded-full border border-white/10">Performance Frameworks</span>
          <h2 className="text-2xl sm:text-3xl font-black text-white mt-4 mb-10">Predictable Parameters for Scale Operations</h2>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-left">
            {HIGHLIGHT_METRICS.map((hm, i) => (
              <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center">
                <div className="text-3xl font-black text-white mb-1">{hm.val}</div>
                <div className="text-xs font-bold text-gray-400 uppercase tracking-wider">{hm.label}</div>
              </div>
            ))}
          </div>
        </div>
